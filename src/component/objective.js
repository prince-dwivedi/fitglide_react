...
const Objective = ({ visitorName }) => {
    ...
    return (
        <div className={`objective ${fade ? 'fade-out' : ''}`}>
            ...
            <div className={`button-container ${fade ? 'fade-out' : ''}`} id='selection-button'>
                <button className="objective-button weight-loss-button" onClick={handleButtonClick}>Weight Loss</button>
                <button className="objective-button diabetes-button" onClick={handleButtonClick}>Diabetes Management</button>
            </div>
        </div>
    );
};
